...
class KeroseneNocController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('citizen.temporary..kerosene.kerosene', compact('districts','tehsil','block','panchayat'));
            
    }
    public function addKeroseneBasicDetails(Request $request)
    {
        ...
        $result = $this->commonModel->insertData('temp_kerosene_applications', $data);
        if ($result == 1)
        {
    ...
    public function addKeroseneApplicantDetails(Request $request)
    {
        ...
        $result = $this->commonModel->updateDataByOneCondition('temp_kerosene_applications', ['application_no' => $request->application_no], $data);
        if ($result == 1)
        {
    ...
    public function addKeroseneOrganizingDetails(Request $request)
    {
        $user_id = Auth::user()->id;
        $organizing_address = [];
    ...
    public function addKeroseneOrganizerDetails(Request $request)
    {
        $user_id = Auth::user()->id;
    ...
    public function addKeroseneErectorDetails(Request $request)
    {
        $user_id = Auth::user()->id;
    ...
    public function addKeroseneCoordinatorDetails(Request $request)
    {
        $user_id = Auth::user()->id;
    ...
    public function addKeroseneProjectDetails(Request $request)
    {
        $user_id = Auth::user()->id;
        $project_area_detail = [];

        $project_area_detail['capacity'] = $request->capacity;
        $project_area_detail['tank_drum'] = $request->tank_drum;
        $project_area_detail['type_class'] = $request->type_class;
        $project_area_detail['purpose'] = $request->purpose;
        $data = [
    ...
    public function addKeroseneAttachmentsDetails(Request $request)
    {
        $reference_letter = '';
        $photo_id_card = '';
        $aadhar_card = '';
        ...
        if ($request->hasFile('photo_id_card')) {
            $file = $request->file('photo_id_card'); // Correctly get the file
            $fileName = time() . '.' . $file->getClientOriginalExtension();
            $file->move(public_path('citizen/file'), $fileName);
            $photo_id_card = 'public/citizen/file/' . $fileName;
        }
        
        if ($request->hasFile('aadhar_card')) {
            $file = $request->file('aadhar_card'); // Correctly get the file
            $fileName = time() . '.' . $file->getClientOriginalExtension();
            $file->move(public_path('citizen/file'), $fileName);
            $aadhar_card = 'public/citizen/file/' . $fileName;
        }
