                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6 col-sm-6 col-xs-12 occupency_div" id="occupency_div7" style="padding-right:0;display:none;">
                                <div class="form-group">
                                    ...
                                    <div class="radio-toolbar">
                                        <input type="radio" id="yes" name="is_hazardous_material" value="yes" onclick="chooseHazardous(this);" @checked(old('building_ownership', data_get($application,'building_ownership')) == 'tan')>
                            ...
                            <div class="col-md-3 col-sm-6 col-xs-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control js-example-basic-single" name="person_appointed" id="person_appointed" required>
                                        ...
                                        <option value="CEO" @selected(old('person_appointed', $contact_person['person_appointed'] ?? '' )=='Director' )>CEO</option>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on("click", "#submitBasic", function(e) {
            ...
            // if ($('[name="occupancy_value"]').length && !formData.occupancy_value) {
            //     errors.push({
            //         id: "error", // or dedicated occupancy error span
            //         msg: "Occupancy value is required"
            //     });
            // }
        ...
        // autfill data
        // $(document).on('change', '.point_of_contact', function(){
        //     //alert($(this).val());
        //     let checkValue = $(this).val();
        //     if(checkValue=='yes')
        //     {
        //         let 1_salutation = $('#salutation').val();
        //         let 1_first_name = $('#first_name').val();
        //         let 1_middle_name = $('#middle_name').val();
        //         let 1_last_name = $('#"last_name').val();
        //         let 1_mobile_no = $('#"mobile_no').val();
        //         let 1_email = $('#email').val();


        //         $('#con_salutation').val(1_salutation);
        //         $('#con_first_name').val(1_first_name);
        //         $('#con_middle_name').val(1_middle_name);
        //         $('#con_last_name').val(1_last_name);
        //         $('#con_mobile_no').val(1_mobile_no);
        //         $('#con_email').val(1_email);
...
<script>
    ...
    //     let ruralUrban = "{{ data_get($application, 'rural_urban') }}"; // urban / rural
    //     let tehsilId = "{{ data_get($application, 'tehsil_id') }}";
    //     let urbanBodyId = "{{ data_get($application, 'urban_body_id') }}";
    //     let wardId = "{{ data_get($application, 'ward_id') }}";
    //     let blockId = "{{ data_get($application, 'block_id') }}";
    //     let panchayatId = "{{ data_get($application, 'panchayat_id') }}";
        
    //     let districtId = "{{ $lockedDistrictId ?? '' }}";

    //     if (!districtId) {
    //         districtId = "{{ data_get($application, 'district_id') ?? Auth::user()->district_id }}";
    //     }
    ...
    //     } else {
    //         $('#rular').prop('checked', true);
    //     }

    //     chooseRularUrban();

    //     /* -----------------------------
    //     2️⃣ Load Tehsil & Block
    //     ------------------------------ */
    ...
    //     }, function(response) {
    //         $('#tehsil_id').html(response);
    //         if (tehsilId) $('#tehsil_id').val(tehsilId).trigger('change');
    //     });
    ...
    //     }, function(response) {
    //         $('#block_id').html(response);
    //         if (blockId) $('#block_id').val(blockId).trigger('change');
    //     });

    //     /* -----------------------------
    //     3️⃣ If URBAN → Load Urban Body
    //     ------------------------------ */
    //     // if (ruralUrban === 'urban' && tehsilId) {

    //         $.post("getUrbanBodyByTehsil", {
    //             district_id: districtId
    //         }, function(response) {
    //             $('#urban_body_id').html(response);
    //             if (urbanBodyId) $('#urban_body_id').val(urbanBodyId).trigger('change');
    //         });
    //     // }
    ...
    //     if (urbanBodyId) {
    //         $.post("getWardByUrbanBody", {
    //             urban_body_id: urbanBodyId
    //         }, function(response) {
    //             $('#ward_id').html(response);
    //             if (wardId) $('#ward_id').val(wardId);
    //         });
    //     }
    ...
    //             if (panchayatId) $('#panchayat_id').val(panchayatId).trigger('change');
    //         });
    //     }
    ...
    // $(document).ready(function () {
    //     let ruralUrban   = "{{ data_get($application, 'rural_urban') }}";
    //     let tehsilId     = "{{ data_get($application, 'tehsil_id') }}";
    //     let urbanBodyId  = "{{ data_get($application, 'urban_body_id') }}";
    //     let wardId       = "{{ data_get($application, 'ward_id') }}";
    //     let blockId      = "{{ data_get($application, 'block_id') }}";
    ...
    //     // SAFEST boolean handling
    //     let hasApplication     = @json(!empty($application));
    //     let hasLockedDistrict = @json(!empty($lockedDistrictId));
    //     let districtId = "{{ $lockedDistrictId ?? '' }}";

    //     // let districtId = "{{ $lockedDistrictId ?? data_get($application,'district_id') ?? Auth::user()->district_id }}";
    ...
    //         $.post("getTehsilByDistrict", { district_id: districtId }, function (response) {
    //             $('#tehsil_id').html(response);

    //             if (tehsilId) {
    //                 $('#tehsil_id').val(tehsilId).trigger('change');

    //                 if (ruralUrban === 'urban') {
    //                     $.post("getUrbanBodyByTehsil", { tehsil_id: tehsilId }, function (response) {
    //                         $('#urban_body_id').html(response);

    //                         if (urbanBodyId) {
    //                             $('#urban_body_id').val(urbanBodyId).trigger('change');

    //                             $.post("getWardByUrbanBody", { urban_body_id: urbanBodyId }, function (response) {
    //                                 $('#ward_id').html(response);
    //                                 if (wardId) $('#ward_id').val(wardId);
    //                             });
    //                         }
    ...
    //         $.post("getBlockByDistrict", { district_id: districtId }, function (response) {
    //             $('#block_id').html(response);

    //             if (blockId) {
    //                 $('#block_id').val(blockId).trigger('change');

    //                 $.post("getPanchayatByBlock", { block_id: blockId }, function (response) {
    //                     $('#panchayat_id').html(response);
    //                     if (panchayatId) $('#panchayat_id').val(panchayatId);
    //                 });
    //             }
    //         });
    //     }

    //     if (!hasApplication && hasLockedDistrict) {
    ...
    //         $('#tehsil_id, #urban_body_id, #ward_id, #block_id, #panchayat_id')
    //             .prop('disabled', true);
    //         $('#village, #street').prop('readonly', true);
    //     }
    ...
    //     let projectId = $('#selected_project').val();     // ⬅ from hidden input
    //     let selectedSubcategory = $('#selected_subcategory').val();
    //     let selectedCategory = $('#selected_category').val();
    //     let _token = $('input[name="_token"]').val();
    //     if (projectId != '' && _token != '') {

    //         // SET project dropdown selected
    //         $("#project_id").val(projectId);

    //         $.ajax({
    //             url: "{{ route('getSubCategoryByProject') }}",
    //             type: "POST",
    //             data: {
    //                 _token: _token,
    ...
    //                 let objSubCategory = JSON.parse(response);
    //                 let html = '<option value="">---- Select Sub Category ----</option>';

    //                 if (objSubCategory.status == 1) {

    //                     $.each(objSubCategory.data, function(key, value) {
    //                         html += `<option value="${value.id}" ${value.id == selectedSubcategory ? 'selected' : ''}>${value.name}</option>`;
    //                     });
    //                 }

    //                 $('#subcategory_id').html(html);

    //                 // If editing, load category next
    //                 if (selectedSubcategory != '') {
    //                     loadCategoryBySubcategory();
    //                 }
    //             }
    //         });
    //     }
