        ...
        <div id="content" style="width: 95%;border: 1px solid #000;;margin: 0px auto;padding: 5px 10px;box-shadow: none;border-bottom:1px solid #000;height:auto;margin-bottom:20px; margin-top:20px;">
            ...
            <div style="text-align: center;margin-bottom: 10px;display: flex;">
                ...
                <div style="width:60%;text-align:center;">
                    ...
                    <p style="line-height:20px; text-align:center;color:#000; font-weight: bold; font-size: 14px; margin-top:10px;">
                ...
                <div style="width:20%;text-align:right;margin-top:10px;">
                    <img
                        src="{{ asset('public/qrcodes/' . $applicationDetail[0]->application_qr_code) }}"
                        style="width:130px;height:auto;display:block;margin:0 auto;object-position:center;"
            ...
            <table style="width: 100%; border-collapse: collapse; background-color: #ffffff;">
                ...
                <tbody>
                    ...
                    @if(json_decode($applicationDetail[0]->physical_ins)->line=='yes')
                    <tr>
            ...
            <table style="width: 100%; border-collapse: collapse; margin-top: 0px; background-color: #ffffff;">
                ...
                <tbody>
                    ...
                    @endif
                    @if(isset($fire['type_diesel_under_ground_tank']) && $fire['type_diesel_under_ground_tank'] !='undefined')
                    <tr>
                    ...
                    @endif
                    @if(isset($fire['type_jockey_under_ground_tank']) && $fire['type_jockey_under_ground_tank'] !='undefined')
                    ...
                    @if(!in_array($fire['automatic_sprinkler'] ?? '', ['Not Required', 'Not Provided']))
                    <tr>
            ...
            @if($applicationDetail[0]->status === 'approved')
                ...
                @php
                    ...
                    /**
                    * Normalize OLD + NEW reason formats
                    * OLD  -> JSON string with reason1..reason5
                    * NEW  -> JSON array of strings
                    */
                    function normalizeReasons($reasonRaw)
                    {
                        $result = [];

                        if (empty($reasonRaw)) return $result;

                        // NEW FORMAT (array)
                        if (is_array($reasonRaw)) {
                            foreach ($reasonRaw as $line) {
                                if (!empty(trim($line))) {
                                    $result[] = trim($line);
                                }
                            }
                    ...
                    /**
                    * Collect remarks from a source (DD / CFO / FSO)
                    */
                    function collectRemarks($json, &$finalConditions)
                    {
                        ...
                        foreach (json_decode($json) as $item) {
                            ...
                            // Conditions
                            $reasons = normalizeReasons($item->reason ?? null);
                            $finalConditions = array_merge($finalConditions, $reasons);

                            // Free-text remark
                            if (!empty(trim($item->remark ?? ''))) {
                                $finalConditions[] = trim($item->remark);
                            }
                        }
                    }

                    // 🔹 Collect from DD / CFO / FSO
                    collectRemarks($applicationDetail[0]->remark_by_dd  ?? null, $finalConditions);
                    collectRemarks($applicationDetail[0]->remark_by_cfo ?? null, $finalConditions);
                    collectRemarks($applicationDetail[0]->remark_by_fso ?? null, $finalConditions);

                    // 🔹 Remove duplicates + reindex
                    $finalConditions = array_values(array_unique($finalConditions));
                @endphp

                @if(count($finalConditions) > 0)
                <div style="justify-content: space-between; padding:5px 10px; margin-top:10px;">
                    <h6 style="font-size:11px;"><strong>Required Provision / Remarks</strong></h6>

                    <ol style="margin-left:18px;">
                        @foreach($finalConditions as $line)
                            <li style="font-size:10px; line-height:15px;">
                                {{ $line }}
                            </li>
                        @endforeach
                    </ol>
                ...
                <div style="width: 33.333%; margin-left:8%;color:#000;">
                    ...
                    <p style="color:#000;">
                        <img src="{{ asset($applicationDetail[0]->fso_signature) }}" alt="client" width="70" height="70" class="shadow-sm mr-3" />
                            <br>
                            {{$applicationDetail[0]->fso_name ?? 'NA'}}
                            <br>
                            {{$applicationDetail[0]->fso_designation}}
                            <br>
                            {{ ucfirst($station[0]->name ?? 'NA') }}
                            <br>
                            {{ ucfirst($district[0]->name)}}
                            <br>
