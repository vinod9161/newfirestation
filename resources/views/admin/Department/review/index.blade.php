...
<div class="d-md-flex d-block align-items-center justify-content-between my-4">
    <div>
        <h5 class="main-content-title text-default  fs-24  mg-b-4 mb-0">Manage Review Officer</h5>
    </div>
    <div class="d-flex app-header-btn">
        ...
        <div>
            ...
            <a href="<?php echo route('admin.reviewadd');?>" class="btn ripple btn-wave  btn-success mb-0">
                <i class="fe fe-plus me-1"></i> Add New Review Officer
            </a>
        </div>
    </div>
</div>
        ...
        <form action="{{ route('admin.reviewfilter') }}" method="GET" class="advanced-search br-3">
            <div class="advanced-search br-3">
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group mb-lg-0">
                                    <label>Name :</label>
                                    <input type="text" class="form-control" name="filter_name" value="{{ request('filter_name') }}" id="filter_name" placeholder=" Enter Name">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group mb-lg-0">
                                    <label>Email :</label>
                                    <input type="text" class="form-control" name="filter_email" value="{{ request('filter_email') }}" id="filter_email"
                ...
                <button type="submit" class="btn btn-primary">Apply</button>
                    <a href="{{ route('admin.review') }}" class="btn btn-secondary">Reset</a>
                </div>
            </div>
        </form>
</div>
                ...
                <div class="table-responsive">
                    ...
                    <table id="datatable-basic" class="table table-bordered text-nowrap w-100">
                        <thead>
                            <tr role="row">
                                ...
                                <th class="sorting" tabindex="0" aria-controls="employee-table" rowspan="1" colspan="1"
                                    ...
                                    Name<div style="height: 25px;"></div>
                                </th>
                                <th class="sorting" tabindex="0" aria-controls="employee-table" rowspan="1" colspan="1"
                                    aria-label="Name: activate to sort column ascending" style="width: 153px;">Email<div
                                        style="height: 25px;"></div>
                                </th>
                                <th class="sorting" tabindex="0" aria-controls="employee-table" rowspan="1" colspan="1"
                                    aria-label="Gender: activate to sort column ascending" style="width: 67px;">Mobile
                                </th>


                                <th class="sorting" tabindex="0" aria-controls="employee-table" rowspan="1" colspan="1"
                                    aria-label="Gender: activate to sort column ascending" style="width: 67px;">District
                                </th>
                        ...
                        <tbody>
                            ...
                            <?php //echo "<pre>"; print_r($reviewOfficer); die;?>

                            @foreach ($reviewOfficer as $index => $district)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>{{ $district ->name}}</td>
                                <td>{{ $district->email }}</td>
                                <td>{{ $district->number }}</td>
                                <td>{{ ucfirst($district->district->name ?? "")}}</td>
                                <td>{{ $district->status == 1 ? 'Active' : 'Inactive' }}</td>
                                <td class="text-right">
                                    <a href="{{ route('admin.editreview', $district->id) }}"
                                        class="btn btn-primary btn-sm">Edit</a>

                                    <form action="{{ route('admin.deletereview', $district->id) }}" method="POST"
                                        style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm"
                                            onclick="return confirm('Are you sure you want to delete this Review Officer?');">
