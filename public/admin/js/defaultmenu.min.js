...
(() => {
  ...
  if (
    !localStorage.getItem("dayonelayout") &&
    !localStorage.getItem("dayonenavstyles") &&
    !localStorage.getItem("dayoneverticalstyles") &&
      ...
      switch (verticalStyle) {
        ...
        case "overlay":
          ...
          if (html.getAttribute('data-toggled') === 'icon-overlay-close') {
            ...
            if (window.innerWidth >= 992) {
              if (!localStorage.getItem("dayonelayout")) {
...
function setNavActive() {
  ...
  sidemenuItems.forEach(e => {
    ...
    if (e.getAttribute('href') === currentPath) {
      ...
      // Used to add the active for the current link "a"
      e.classList.add('active');
      // Used to add the active for the current link "li"
      e.parentElement.classList.add('active');
      // Used to access the parent ul.
      let parent = e.closest('ul');
      let parentNotMain = e.closest('ul:not(.main-menu)');
      let hasParent = true;
      // while (hasParent) {
      if (parent) {
        // Used to add the active for the parent "ul"
        parent.classList.add('active');
        if (parent.previousElementSibling) {
          // Used to add the active for the parent sibling "a"
          parent.previousElementSibling.classList.add('active');
        }
        // Used to add the active for the parent sibling "ul"
        parent.parentElement.classList.add('active');
        // Checking is there was any "ul" using "has-sub"
        let parentEle = parent.parentElement;
        while(parentEle){
          addActiveForTopLevelParent(parentEle);
          parentEle = parentEle.parentElement.closest('.slide');
        }
...
function addActiveForTopLevelParent(parent){
  let elementRef = parent.parentElement.parentElement;
  // Adding active for the closest has-sub "ul"
  elementRef.classList.add("open", "active")
  // Adding active for the closest has-sub "a"
  elementRef.firstElementChild.classList.add("active")
  // Adding style block for the closest has-sub "ul"
  elementRef.children[1] ? elementRef.children[1].style.display = "block" : '',
  Array.from( elementRef.children[1]?.children || []).map((element) => {
    if (element.classList.contains("active")) {
      // Adding style block for the closest has-sub "ul" children ul.
      element.children[1] ? element.children[1].style.display = "block" : '';
    }
  })
}
